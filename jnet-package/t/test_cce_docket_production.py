...
def test_single_request_pipeline(jnetclient, single_docket_request_check_status):
    ...
    fullfetch = jnetclient.fetch_docket_data(docket_number, timeout = 120)
...
def test_server_cert_errors(jnetclient):

    # we're going to use the real jnetclient to find incorrect certificates for this test.

    badclient = jnet.CCE(
        server_certificate = jnetclient.find_certificate("ws.jnet.beta.pa.gov.crt"),
        endpoint = 'jnet',
    )
    # make sure that we get an error if the server certificate references teh wrong server
    #badclient.server_certificate = "cert/ws.jnet.beta.pa.gov_2022/ws.jnet.beta.pa.gov.combined.crt"
    import requests
    ...
    badclient.server_certificate = jnetclient.find_certificate("Intermediate.crt")
    with pytest.raises(requests.exceptions.SSLError):
        resp = badclient.request_docket( test_docket_number)

    try:
        badclient.server_certificate = jnetclient.find_certificate("webservice.cert.pem")
        with pytest.raises(requests.exceptions.SSLError):
            resp = badclient.request_docket( test_docket_number)
    except FileNotFoundError:
        #No random pem certificates found
        pass
...
def test_client_setup_errors(jnetclient):
    ...
    # See what happens if the client certificate file referecend exists
    # and is a legitimate certificate but is wrong because it's for the server and not the client
    jnetclient.client_certificate = jnetclient.find_certificate("ws.jnet.pa.gov.crt")
